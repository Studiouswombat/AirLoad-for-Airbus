...
import yaml, os, sys, argparse, math, time, copy, json, datetime
...
# ══════════════════════════════════════════════════════════════════════════════
# EXPORT ENGINE  —  JSON + PDF  (Load Sheet for Load Master)
# ══════════════════════════════════════════════════════════════════════════════

def build_export_json(plan: dict) -> dict:
    """
    Construct the JSON payload used for logging / documentation.
    Contains: flight details, fuel details, weight breakdown, full cargo manifest
    with zone assignments and CG contributions.
    """
    flt  = plan.get("flight_plan") or {}
    wb   = plan.get("weight_breakdown") or {}
    ...
    for e in plan.get("loading_plan", []):
        cargo_rows.append({
            ...
            "assigned_zone":    e.get("assigned_zone") or "UNASSIGNED",
            "hold":             ("Forward Hold" if (e.get("assigned_zone") or "").startswith("FWD")
                                 else "Aft Hold"  if (e.get("assigned_zone") or "").startswith("AFT")
                                 else "Bulk Hold" if (e.get("assigned_zone") or "") == "BULK_01"
                                 else "Unassigned"),
            "zone_x_m":        (ZONE_DEFS[e["assigned_zone"]].x_position_m
                                 if e.get("assigned_zone") and e["assigned_zone"] in ZONE_DEFS
                                 else None),
            "cg_after_placement_pct_mac": e.get("cg_after_placement"),
            "manual_lock":      e.get("manual_lock", False),
        })

    return {
        "document_type":    "AIRLoad Cargo Configuration",
        "generated_utc":    now,
        "aircraft":         plan.get("aircraft", "Airbus A350-1000"),
        "flight": {
            "flight_number":   flt.get("flight_number", ""),
            "origin":          flt.get("origin", ""),
            "destination":     flt.get("destination", ""),
            "flight_hours":    flt.get("flight_hours", ""),
            "alternate_hours": flt.get("alternate_hours", ""),
        },
        "fuel": {
            "fuel_loaded_kg":       wb.get("fuel_kg"),
            "burn_rate_kg_per_hr":  flt.get("burn_rate_kg_per_hr"),
            "wing_tank_cg_x_m":    flt.get("wing_tank_cg_x_m"),
        },
        "weight_breakdown": {
            "operating_empty_weight_kg": wb.get("empty_aircraft_kg"),
            "passengers_kg":             wb.get("passengers_kg"),
            "fuel_kg":                   wb.get("fuel_kg"),
            "cargo_kg":                  wb.get("cargo_kg"),
            "ramp_weight_kg": (
                (wb.get("empty_aircraft_kg") or 0) +
                (wb.get("passengers_kg")     or 0) +
                (wb.get("fuel_kg")           or 0) +
                (wb.get("cargo_kg")          or 0)
            ),
        },
        "centre_of_gravity": {
            "ramp_cg_pct_mac":    plan.get("final_cg_pct_mac"),
            "cg_fwd_limit":       CG_FWD,
            "cg_aft_limit":       CG_AFT,
            "cg_target":          CG_TGT,
            "status":             compute_cg_status(plan.get("final_cg_pct_mac")),
        },
        "cargo_manifest":       cargo_rows,
        "hold_weights": {
            "forward_hold_kg":    hold_weights(plan)[0],
            "aft_hold_kg":        hold_weights(plan)[1],
            "bulk_hold_kg":       hold_weights(plan)[2],
            "forward_limit_kg":   FWD_HOLD_LIMIT_KG,
            "aft_limit_kg":       AFT_HOLD_LIMIT_KG,
            "bulk_limit_kg":      BULK_LIMIT_KG,
        },
    }


# ── PDF colour palette (CMYK-safe hex equivalents of UI theme) ───────────────
_PDF_DARK    = colors.HexColor("#1a1f2e")
_PDF_NAVY    = colors.HexColor("#222840")
_PDF_BORDER  = colors.HexColor("#3a4468")
_PDF_ACCENT  = colors.HexColor("#4a9eff")
_PDF_TEXT    = colors.HexColor("#dde4f0")
_PDF_MUTED   = colors.HexColor("#7a8aaa")
_PDF_GREEN   = colors.HexColor("#2ecc71")
_PDF_YELLOW  = colors.HexColor("#f0b429")
_PDF_RED     = colors.HexColor("#e74c3c")
_PDF_ORANGE  = colors.HexColor("#f39c12")
_PDF_WHITE   = colors.white
_PDF_BLACK   = colors.HexColor("#0d1117")

_CARGO_COLS_HEX = [
    "#2980b9","#1abc9c","#8e44ad","#e67e22","#27ae60","#c0392b",
    "#16a085","#d35400","#2c3e50","#f39c12","#7f8c8d","#6c3483",
    "#117a65","#873600","#1a5276",
]


def _hex_to_rl(h: str):
    return colors.HexColor(h)


def _draw_aircraft_pdf(c_obj, x0, y0, w, h, zone_map, item_colours):
    """
    Draw a top-view A350-1000 silhouette + cargo slots onto a reportlab canvas.

    c_obj  — reportlab canvas object
    x0,y0  — bottom-left origin of the bounding box (reportlab coords: y=0 at bottom)
    w, h   — width / height of the drawing area in points
    """
    # ── coordinate helpers ───────────────────────────────────────────────────
    X_NOSE, X_TAIL = 3.0, 52.0
    mx = 30          # left/right margin in pts
    usable_w = w - 2 * mx
    cy_rl = y0 + h * 0.50   # centre y (reportlab: up is positive)

    def ac_x(x_m):
        return x0 + mx + (x_m - X_NOSE) / (X_TAIL - X_NOSE) * usable_w

    def fus_half(x_m):
        rel = (x_m - X_NOSE) / (X_TAIL - X_NOSE)
        if rel < 0.05:  return (rel / 0.05) * 14
        if rel > 0.85:  return max(2, 14 * (1 - (rel - 0.85) / 0.15))
        return 14
    ...
    upper_pts, lower_pts = [], []
    for i in range(N + 1):
        xm = X_NOSE + i / N * (X_TAIL - X_NOSE)
        px = ac_x(xm); hw = fus_half(xm)
        upper_pts.append((px, cy_rl + hw))
        lower_pts.append((px, cy_rl - hw))

    all_pts = upper_pts + list(reversed(lower_pts))
    c_obj.setFillColor(colors.HexColor("#21262d"))
    c_obj.setStrokeColor(colors.HexColor("#3a4468"))
    ...
    for pt in all_pts[1:]:
        path.lineTo(*pt)
    path.close()
    c_obj.drawPath(path, fill=1, stroke=1)

    # ── wings ────────────────────────────────────────────────────────────────
    wx1 = ac_x(22); wx2 = ac_x(33)
    tip_dy = min(h * 0.38, 80)
    c_obj.setFillColor(colors.HexColor("#1c2535"))
    c_obj.setStrokeColor(colors.HexColor("#3a4468"))
    c_obj.setLineWidth(0.6)
    for sign in (1, -1):
        pts_w = [
            (wx1, cy_rl + sign * 14),
            (wx2, cy_rl + sign * 14),
            (wx2 + 20, cy_rl + sign * tip_dy),
            (wx1 - 10, cy_rl + sign * tip_dy),
        ]
        path = c_obj.beginPath()
        path.moveTo(*pts_w[0])
        for pt in pts_w[1:]: path.lineTo(*pt)
        path.close()
        c_obj.drawPath(path, fill=1, stroke=1)

    # ── horizontal stabilisers ───────────────────────────────────────────────
    sx1 = ac_x(44); sx2 = ac_x(49)
    stab_dy = min(h * 0.14, 36)
    c_obj.setFillColor(colors.HexColor("#21262d"))
    for sign in (1, -1):
        pts_s = [
            (sx1, cy_rl + sign * 8),
            (sx2, cy_rl + sign * 8),
            (sx2 + 8, cy_rl + sign * stab_dy),
            (sx1,     cy_rl + sign * stab_dy),
        ]
        path = c_obj.beginPath()
        path.moveTo(*pts_s[0])
        for pt in pts_s[1:]: path.lineTo(*pt)
        path.close()
        c_obj.drawPath(path, fill=1, stroke=1)
    ...
    c_obj.setDash(4, 4)
    c_obj.line(ac_x(X_NOSE), cy_rl, ac_x(X_TAIL), cy_rl)
    c_obj.setDash()

    # ── cargo zone slots ─────────────────────────────────────────────────────
    slot_h  = 22
    gap_cy  = 12
    slot_w  = max(20, usable_w / (X_TAIL - X_NOSE) * 1.5)

    def _slot(zone_id, cx_canvas, upper):
        items_here = zone_map.get(zone_id, [])
        occupied   = len(items_here) > 0

        x1 = cx_canvas - slot_w / 2
        if upper:
            y_bot = cy_rl + gap_cy; y_top = y_bot + slot_h
        else:
            y_top = cy_rl - gap_cy; y_bot = y_top - slot_h

        if occupied:
            item_name = items_here[0]["item"]
            hex_col   = item_colours.get(item_name, "#4a9eff")
            c_obj.setFillColor(colors.HexColor(hex_col))
            c_obj.setStrokeColor(colors.HexColor("#ffffff"))
            c_obj.setLineWidth(0.5)
        else:
            c_obj.setFillColor(colors.HexColor("#252d45"))
            c_obj.setStrokeColor(colors.HexColor("#3a4468"))
            c_obj.setLineWidth(0.4)

        c_obj.roundRect(x1, min(y_bot, y_top), slot_w, slot_h, 2, fill=1, stroke=1)

        short = (zone_id.replace("FWD_0","F").replace("AFT_0","A")
                         .replace("BULK_01","BLK"))
        c_obj.setFont("Helvetica-Bold", 5)
        mid_x = cx_canvas
        mid_y = (y_bot + y_top) / 2

        if occupied:
            c_obj.setFillColor(colors.white)
            c_obj.drawCentredString(mid_x, mid_y + 5, short)
            nm = items_here[0]["item"][:9]
            c_obj.setFont("Helvetica", 4)
            c_obj.drawCentredString(mid_x, mid_y + 1, nm)
            c_obj.drawCentredString(mid_x, mid_y - 4,
                                    f"{items_here[0]['weight_kg']:.0f}kg")
        else:
            c_obj.setFillColor(colors.HexColor("#7a8aaa"))
            c_obj.drawCentredString(mid_x, mid_y - 2, short)

        # connector line
        c_obj.setStrokeColor(colors.HexColor("#3a4468"))
        c_obj.setLineWidth(0.4)
        c_obj.setDash(2, 3)
        c_end = y_top if upper else y_bot
        c_obj.line(mid_x, cy_rl, mid_x, c_end)
        c_obj.setDash()

    for n in range(1, 5):
        xm  = {1:15.0,2:17.0,3:19.0,4:21.0}[n]
        cxc = ac_x(xm)
        _slot(f"FWD_0{n}L", cxc, upper=True)
        _slot(f"FWD_0{n}R", cxc, upper=False)
    for n in range(1, 5):
        xm  = {1:31.0,2:33.0,3:35.0,4:37.0}[n]
        cxc = ac_x(xm)
        _slot(f"AFT_0{n}L", cxc, upper=True)
        _slot(f"AFT_0{n}R", cxc, upper=False)
    _slot("BULK_01", ac_x(40.0), upper=False)

    # ── hold labels ──────────────────────────────────────────────────────────
    fwd_w, aft_w, bulk_w = hold_weights({"loading_plan": [
        i for items in zone_map.values() for i in items if i.get("assigned_zone")
    ]})
    label_y = cy_rl - gap_cy - slot_h - 10

    c_obj.setFont("Helvetica-Bold", 6)
    c_obj.setFillColor(colors.HexColor("#4a9eff"))
    c_obj.drawCentredString((ac_x(15)+ac_x(21))/2, label_y,
                             f"FWD HOLD  {fwd_w:,.0f} kg")
    c_obj.drawCentredString((ac_x(31)+ac_x(37))/2, label_y,
                             f"AFT HOLD  {aft_w:,.0f} kg")
    c_obj.drawCentredString(ac_x(40), label_y, f"BULK  {bulk_w:.0f} kg")

    # ── CG marker on fuselage ────────────────────────────────────────────────
    cg_val = None
    for items in zone_map.values():
        pass  # we'll get it from caller context below — see wrapper

    return label_y  # return lowest y used so caller knows safe text region


def generate_pdf_loadsheet(plan: dict, item_colours: dict, out_path: str):
    """
    Generate a landscape A4 PDF load sheet for the load master / pilot sign-off.

    Page 1: Header + aircraft diagram + CG gauge
    Page 2: Full cargo manifest table + weight breakdown + signatures
    """
    ...
    fwd_w, aft_w, bulk_w = hold_weights(plan)
    final_cg = plan.get("final_cg_pct_mac")
    cg_status = compute_cg_status(final_cg)
    cg_col_rl = {"green": _PDF_GREEN, "yellow": _PDF_YELLOW,
                 "red":   _PDF_RED,   "muted": _PDF_MUTED}.get(cg_status, _PDF_MUTED)

    c = rl_canvas.Canvas(out_path, pagesize=landscape(A4))

    # ══════════════════════════════════════════════════════════════════════════
    # PAGE 1 — Aircraft Diagram + Header
    # ══════════════════════════════════════════════════════════════════════════
    ...
    # Logo / title
    c.setFillColor(_PDF_ACCENT)
    c.setFont("Helvetica-Bold", 20)
    c.drawString(MARGIN, PAGE_H - 18 * mm, "✈  AIRLoad")
    c.setFillColor(_PDF_TEXT)
    c.setFont("Helvetica", 11)
    c.drawString(MARGIN, PAGE_H - 26 * mm, "A350-1000  ·  Cargo Loading Configuration  ·  Load Master Sign-Off Sheet")
    c.setFillColor(_PDF_MUTED)
    c.setFont("Helvetica", 8)
    c.drawString(MARGIN, PAGE_H - 33 * mm, f"Generated: {now}     Document type: FINAL CARGO CONFIGURATION")

    # Flight details — right side of header
    fl_num = f"{flt.get('flight_number','')}  {flt.get('origin','')} → {flt.get('destination','')}"
    c.setFillColor(_PDF_TEXT)
    c.setFont("Helvetica-Bold", 14)
    c.drawRightString(PAGE_W - MARGIN, PAGE_H - 18 * mm, fl_num)
    c.setFont("Helvetica", 9)
    c.setFillColor(_PDF_MUTED)
    c.drawRightString(PAGE_W - MARGIN, PAGE_H - 25 * mm,
                      f"Block time: {flt.get('flight_hours','—')} h     "
                      f"Fuel loaded: {wb.get('fuel_kg',0):,.0f} kg     "
                      f"PAX: {wb.get('passengers_kg',0)/95:.0f}")
    c.drawRightString(PAGE_W - MARGIN, PAGE_H - 32 * mm,
                      f"Burn rate: {flt.get('burn_rate_kg_per_hr',0):,.0f} kg/hr")
    ...
    # ── weight summary tiles (row under header) ───────────────────────────────
    tile_y    = PAGE_H - header_h - 16 * mm
    tile_h    = 14 * mm
    tile_gap  = 4 * mm
    avail_w   = PAGE_W - 2 * MARGIN
    oew  = wb.get("empty_aircraft_kg", 0)
    pax  = wb.get("passengers_kg",     0)
    fuel = wb.get("fuel_kg",           0)
    cgo  = wb.get("cargo_kg",          0)
    ramp = oew + pax + fuel + cgo

    tiles = [
        ("OEW",       f"{oew:,.0f} kg",  "#4a9eff"),
        ("Passengers",f"{pax:,.0f} kg",  "#4a9eff"),
        ("Fuel (FOB)",f"{fuel:,.0f} kg", "#f0b429"),
        ("Cargo",     f"{cgo:,.0f} kg",  "#2ecc71"),
        ("Ramp Weight",f"{ramp:,.0f} kg","#e74c3c"),
        ("FWD Hold",  f"{fwd_w:,.0f}/{FWD_HOLD_LIMIT_KG:,} kg",
                      "#e74c3c" if fwd_w > FWD_HOLD_LIMIT_KG else "#2ecc71"),
        ("AFT Hold",  f"{aft_w:,.0f}/{AFT_HOLD_LIMIT_KG:,} kg",
                      "#e74c3c" if aft_w > AFT_HOLD_LIMIT_KG else "#2ecc71"),
    ]
    n_tiles = len(tiles)
    tw = (avail_w - tile_gap * (n_tiles - 1)) / n_tiles

    for i, (lbl, val, col) in enumerate(tiles):
        tx = MARGIN + i * (tw + tile_gap)
        c.setFillColor(colors.HexColor("#1e2339"))
        c.roundRect(tx, tile_y, tw, tile_h, 3, fill=1, stroke=0)
        c.setStrokeColor(colors.HexColor(col))
        c.setLineWidth(1.5)
        c.roundRect(tx, tile_y, tw, tile_h, 3, fill=0, stroke=1)
        c.setFillColor(colors.HexColor("#7a8aaa"))
        c.setFont("Helvetica", 6)
        c.drawCentredString(tx + tw/2, tile_y + tile_h - 5, lbl)
        c.setFillColor(colors.HexColor(col))
        c.setFont("Helvetica-Bold", 8)
        c.drawCentredString(tx + tw/2, tile_y + 3, val)

    # ── CG status tile (rightmost, larger) ───────────────────────────────────
    cg_str = f"{final_cg:.3f} % MAC" if final_cg is not None else "—"
    cg_hex = {"green":"#2ecc71","yellow":"#f0b429","red":"#e74c3c","muted":"#7a8aaa"
               }.get(cg_status,"#7a8aaa")
    # draw as a pill right-aligned after tiles
    cg_tile_x = MARGIN + len(tiles) * (tw + tile_gap)
    cg_tile_w = PAGE_W - MARGIN - cg_tile_x
    if cg_tile_w > 20:
        c.setFillColor(colors.HexColor("#1e2339"))
        c.roundRect(cg_tile_x, tile_y, cg_tile_w, tile_h, 3, fill=1, stroke=0)
        c.setStrokeColor(colors.HexColor(cg_hex))
        c.setLineWidth(1.5)
        c.roundRect(cg_tile_x, tile_y, cg_tile_w, tile_h, 3, fill=0, stroke=1)
        c.setFillColor(colors.HexColor("#7a8aaa"))
        c.setFont("Helvetica", 6)
        c.drawCentredString(cg_tile_x + cg_tile_w/2, tile_y + tile_h - 5, "Ramp CG")
        c.setFillColor(colors.HexColor(cg_hex))
        c.setFont("Helvetica-Bold", 9)
        c.drawCentredString(cg_tile_x + cg_tile_w/2, tile_y + 3, cg_str)

    # ── CG envelope bar ───────────────────────────────────────────────────────
    gauge_top = tile_y - 4 * mm
    gauge_h   = 10 * mm
    gauge_y   = gauge_top - gauge_h
    gauge_x   = MARGIN
    gauge_w   = PAGE_W - 2 * MARGIN
    cg_range  = CG_AFT - CG_FWD

    def _cg_to_gauge_x(pct):
        return gauge_x + (pct - CG_FWD) / cg_range * gauge_w

    # Background
    c.setFillColor(colors.HexColor("#252d45"))
    c.roundRect(gauge_x, gauge_y, gauge_w, gauge_h, 3, fill=1, stroke=0)
    # Yellow zones
    gx1 = _cg_to_gauge_x(CG_TGT - 2); gx2 = _cg_to_gauge_x(CG_TGT + 2)
    c.setFillColor(_PDF_YELLOW)
    c.rect(gauge_x + 1, gauge_y + 1, max(0, gx1 - gauge_x - 1), gauge_h - 2, fill=1, stroke=0)
    c.rect(gx2, gauge_y + 1, max(0, gauge_x + gauge_w - gx2 - 1), gauge_h - 2, fill=1, stroke=0)
    # Green zone
    c.setFillColor(_PDF_GREEN)
    c.rect(gx1, gauge_y + 1, gx2 - gx1, gauge_h - 2, fill=1, stroke=0)
    # Limit labels
    c.setFillColor(_PDF_RED)
    c.setFont("Helvetica-Bold", 7)
    c.drawRightString(gauge_x - 2, gauge_y + gauge_h/2 - 3, f"◄ {CG_FWD:.0f}%")
    c.drawString(gauge_x + gauge_w + 2, gauge_y + gauge_h/2 - 3, f"{CG_AFT:.0f}% ►")
    # Target tick
    tgt_x = _cg_to_gauge_x(CG_TGT)
    c.setStrokeColor(colors.white)
    c.setLineWidth(1.2)
    c.setDash(3, 2)
    c.line(tgt_x, gauge_y - 1, tgt_x, gauge_y + gauge_h + 1)
    c.setDash()
    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 6)
    c.drawCentredString(tgt_x, gauge_y + gauge_h + 2, f"TGT {CG_TGT:.0f}%")
    # CG marker
    if final_cg is not None:
        cgx = max(gauge_x, min(gauge_x + gauge_w, _cg_to_gauge_x(final_cg)))
        c.setFillColor(colors.HexColor(cg_hex))
        c.setStrokeColor(colors.white)
        c.setLineWidth(0.6)
        ...
        c.setFillColor(colors.HexColor(cg_hex))
        path = c.beginPath()
        path.moveTo(cgx, gauge_y + gauge_h + 1)
        path.lineTo(cgx - d, gauge_y + gauge_h + 1 + d * 1.5)
        path.lineTo(cgx + d, gauge_y + gauge_h + 1 + d * 1.5)
        path.close()
        c.drawPath(path, fill=1, stroke=0)
        c.setStrokeColor(colors.HexColor(cg_hex))
        c.setLineWidth(1.5)
        c.line(cgx, gauge_y, cgx, gauge_y + gauge_h)
        # CG label below bar
        c.setFillColor(colors.HexColor(cg_hex))
        c.setFont("Helvetica-Bold", 8)
        c.drawCentredString(cgx, gauge_y - 9, cg_str)

    c.setFillColor(_PDF_MUTED)
    c.setFont("Helvetica", 6)
    c.drawCentredString(PAGE_W / 2, gauge_y - 16,
                        "CENTRE OF GRAVITY ENVELOPE  —  % MAC")

    # ── aircraft diagram ──────────────────────────────────────────────────────
    diag_top  = gauge_y - 20 * mm
    diag_bot  = 12 * mm
    diag_h    = diag_top - diag_bot
    diag_w    = PAGE_W - 2 * MARGIN

    # Build zone_map from plan for the drawing helper
    zm = build_zone_map(plan)
    _draw_aircraft_pdf(c, MARGIN, diag_bot, diag_w, diag_h, zm, item_colours)

    # ── page number / footer ──────────────────────────────────────────────────
    c.setFillColor(_PDF_MUTED)
    c.setFont("Helvetica", 7)
    c.drawCentredString(PAGE_W / 2, 6 * mm,
                        "Page 1 of 2  ·  AIRLoad A350-1000 Cargo Configuration  ·  CONFIDENTIAL")
    c.setStrokeColor(_PDF_BORDER)
    c.setLineWidth(0.3)
    c.line(MARGIN, 10 * mm, PAGE_W - MARGIN, 10 * mm)

    # ══════════════════════════════════════════════════════════════════════════
    # PAGE 2 — Cargo Manifest Table + Signatures
    # ══════════════════════════════════════════════════════════════════════════
    c.showPage()
    _bg_page()

    # Re-draw narrow header
    c.setFillColor(_PDF_NAVY)
    c.rect(0, PAGE_H - 22 * mm, PAGE_W, 22 * mm, fill=1, stroke=0)
    c.setFillColor(_PDF_ACCENT)
    c.setFont("Helvetica-Bold", 13)
    c.drawString(MARGIN, PAGE_H - 11 * mm, "✈  AIRLoad  —  Cargo Manifest & Load Sheet")
    c.setFillColor(_PDF_TEXT)
    c.setFont("Helvetica", 9)
    c.drawString(MARGIN, PAGE_H - 18 * mm,
                 f"{fl_num}     Aircraft: Airbus A350-1000     {now}")
    c.setFillColor(_PDF_MUTED)
    c.setFont("Helvetica", 7)
    c.drawRightString(PAGE_W - MARGIN, PAGE_H - 11 * mm, "FOR PILOT / LOAD MASTER VERIFICATION")
    c.setStrokeColor(_PDF_BORDER)
    c.setLineWidth(0.5)
    ...
    items    = plan.get("loading_plan", [])
    tbl_x    = MARGIN
    tbl_top  = PAGE_H - 28 * mm
    row_h    = 6.5 * mm

    # Column widths (total ~ avail_w)
    aw = PAGE_W - 2 * MARGIN
    col_w = [aw*0.28, aw*0.10, aw*0.14, aw*0.14, aw*0.16, aw*0.10, aw*0.08]
    headers = ["Cargo Item", "Weight (kg)", "Zone", "Hold", "Zone X (m from nose)",
               "CG After (% MAC)", "Locked"]

    # Header row
    cy_tbl = tbl_top
    c.setFillColor(_PDF_NAVY)
    c.rect(tbl_x, cy_tbl - row_h, aw, row_h, fill=1, stroke=0)
    c.setStrokeColor(_PDF_BORDER)
    c.setLineWidth(0.3)
    c.rect(tbl_x, cy_tbl - row_h, aw, row_h, fill=0, stroke=1)

    cx = tbl_x + 2
    for i, (hdr, cw) in enumerate(zip(headers, col_w)):
        c.setFillColor(_PDF_ACCENT)
        c.setFont("Helvetica-Bold", 7)
        c.drawString(cx + 2, cy_tbl - row_h + 2, hdr)
        cx += cw
    cy_tbl -= row_h

    # Data rows
    for row_idx, e in enumerate(items):
        zone_id   = e.get("assigned_zone") or "—"
        hold_name = ("Forward Hold" if zone_id.startswith("FWD")
                     else "Aft Hold"  if zone_id.startswith("AFT")
                     else "Bulk Hold" if zone_id == "BULK_01"
                     else "Unassigned")
        zone_x    = (f"{ZONE_DEFS[zone_id].x_position_m:.1f}"
                     if zone_id in ZONE_DEFS else "—")
        cg_after  = (f"{e['cg_after_placement']:.3f}"
                     if e.get("cg_after_placement") is not None else "—")
        locked_s  = "🔒" if e.get("manual_lock") else ""

        row_bg = colors.HexColor("#1e2339") if row_idx % 2 == 0 else colors.HexColor("#222840")
        ...
        # Colour swatch for item
        item_hex = item_colours.get(e["item"], "#4a9eff")
        c.setFillColor(colors.HexColor(item_hex))
        c.roundRect(tbl_x + 2, cy_tbl - row_h + 1.5, 3, row_h - 3, 1, fill=1, stroke=0)

        row_vals = [e["item"], f"{e.get('weight_kg',0):,.0f}", zone_id,
                    hold_name, zone_x, cg_after, locked_s]
        cx = tbl_x + 6
        for i, (val, cw) in enumerate(zip(row_vals, col_w)):
            cw_eff = cw - (4 if i == 0 else 0)
            c.setFillColor(_PDF_TEXT)
            c.setFont("Helvetica" if i != 0 else "Helvetica-Bold", 7)
            # Truncate long text
            disp = val if len(val) <= 28 else val[:27] + "…"
            c.drawString(cx + 2, cy_tbl - row_h + 2, disp)
            cx += cw

        cy_tbl -= row_h
        if cy_tbl < 55 * mm:   # near bottom — would overflow
            break

    # ── totals row ────────────────────────────────────────────────────────────
    total_cargo = sum(e.get("weight_kg", 0) for e in items)
    c.setFillColor(colors.HexColor("#1a2f4a"))
    c.rect(tbl_x, cy_tbl - row_h, aw, row_h, fill=1, stroke=0)
    c.setStrokeColor(_PDF_ACCENT)
    c.setLineWidth(0.5)
    c.rect(tbl_x, cy_tbl - row_h, aw, row_h, fill=0, stroke=1)
    c.setFillColor(_PDF_ACCENT)
    c.setFont("Helvetica-Bold", 7)
    c.drawString(tbl_x + 6, cy_tbl - row_h + 2, "TOTAL CARGO")
    c.drawString(tbl_x + col_w[0] + 6, cy_tbl - row_h + 2, f"{total_cargo:,.0f} kg")
    cy_tbl -= row_h

    # ── weight breakdown mini-table on the left ───────────────────────────────
    wb_y   = cy_tbl - 8 * mm
    wb_x   = tbl_x
    wb_w   = aw * 0.40
    wb_h   = 6 * mm
    ...
    c.setFillColor(_PDF_MUTED)
    c.setFont("Helvetica-Bold", 7)
    c.drawString(wb_x, wb_y + 2, "WEIGHT BREAKDOWN")
    c.setStrokeColor(_PDF_BORDER)
    c.setLineWidth(0.3)
    c.line(wb_x, wb_y, wb_x + wb_w, wb_y)
    wb_y -= 1 * mm

    for lbl, val_kg, col_hex in [
        ("Operating Empty Weight (OEW)",  oew,  "#4a9eff"),
        ("Passengers",                    pax,  "#4a9eff"),
        ("Fuel on Board (FOB)",           fuel, "#f0b429"),
        ("Total Cargo",                   cgo,  "#2ecc71"),
        ("RAMP WEIGHT (TOW)",             ramp, "#e74c3c"),
    ]:
        c.setFillColor(colors.HexColor(col_hex))
        c.roundRect(wb_x, wb_y - wb_h + 1, 3, wb_h - 2, 1, fill=1, stroke=0)
        c.setFillColor(_PDF_TEXT)
        c.setFont("Helvetica-Bold" if "RAMP" in lbl else "Helvetica", 7)
        c.drawString(wb_x + 6, wb_y - wb_h/2 - 2, lbl)
        c.setFillColor(colors.HexColor(col_hex))
        c.setFont("Helvetica-Bold", 7)
        c.drawRightString(wb_x + wb_w, wb_y - wb_h/2 - 2, f"{val_kg:,.0f} kg")
        c.setStrokeColor(_PDF_BORDER)
        c.setLineWidth(0.2)
        c.line(wb_x, wb_y - wb_h, wb_x + wb_w, wb_y - wb_h)
        wb_y -= wb_h

    # ── CG / hold summary on the right ───────────────────────────────────────
    sum_x = tbl_x + aw * 0.45
    sum_y = cy_tbl - 8 * mm
    sum_w = aw * 0.55
    ...
    c.setFillColor(_PDF_MUTED)
    c.setFont("Helvetica-Bold", 7)
    c.drawString(sum_x, sum_y + 2, "HOLD WEIGHT SUMMARY & CG")
    c.setStrokeColor(_PDF_BORDER)
    c.setLineWidth(0.3)
    c.line(sum_x, sum_y, sum_x + sum_w, sum_y)

    for lbl, used, limit, col_hex in [
        ("FWD Hold",  fwd_w,  FWD_HOLD_LIMIT_KG, "#e74c3c" if fwd_w  > FWD_HOLD_LIMIT_KG else "#2ecc71"),
        ("AFT Hold",  aft_w,  AFT_HOLD_LIMIT_KG, "#e74c3c" if aft_w  > AFT_HOLD_LIMIT_KG else "#2ecc71"),
        ("Bulk Hold", bulk_w, BULK_LIMIT_KG,      "#e74c3c" if bulk_w > BULK_LIMIT_KG      else "#2ecc71"),
    ]:
        sum_y -= 6 * mm
        bar_w_full = sum_w * 0.45
        bar_fill   = min(1.0, used / limit) * bar_w_full if limit else 0
        c.setFillColor(colors.HexColor("#252d45"))
        c.roundRect(sum_x + sum_w * 0.40, sum_y, bar_w_full, 5, 1, fill=1, stroke=0)
        c.setFillColor(colors.HexColor(col_hex))
        c.roundRect(sum_x + sum_w * 0.40, sum_y, bar_fill, 5, 1, fill=1, stroke=0)
        c.setFillColor(_PDF_TEXT)
        c.setFont("Helvetica", 7)
        c.drawString(sum_x, sum_y, lbl)
        c.setFillColor(colors.HexColor(col_hex))
        c.setFont("Helvetica-Bold", 7)
        c.drawString(sum_x + sum_w * 0.22, sum_y,
                     f"{used:,.0f} / {limit:,} kg")

    sum_y -= 8 * mm
    c.setFillColor(_PDF_TEXT)
    c.setFont("Helvetica", 7)
    c.drawString(sum_x, sum_y, "Ramp CG")
    c.setFillColor(colors.HexColor(cg_hex))
    c.setFont("Helvetica-Bold", 11)
    c.drawString(sum_x + sum_w * 0.30, sum_y - 1, cg_str)
    c.setFillColor(_PDF_MUTED)
    c.setFont("Helvetica", 6)
    c.drawString(sum_x + sum_w * 0.30, sum_y - 9,
                 f"Limits: {CG_FWD:.0f}% – {CG_AFT:.0f}% MAC     Target: {CG_TGT:.0f}% MAC")
    ...
    sig_gap = 6 * mm
    n_sigs  = 3
    sig_w   = (aw - sig_gap * (n_sigs - 1)) / n_sigs
    sig_labels = ["Load Master", "First Officer", "Captain"]

    for i, lbl in enumerate(sig_labels):
        sx = tbl_x + i * (sig_w + sig_gap)
        c.setFillColor(colors.HexColor("#1e2339"))
        c.roundRect(sx, sig_y, sig_w, sig_h, 3, fill=1, stroke=0)
        c.setStrokeColor(_PDF_BORDER)
        c.setLineWidth(0.8)
        c.roundRect(sx, sig_y, sig_w, sig_h, 3, fill=0, stroke=1)
        c.setFillColor(_PDF_MUTED)
        c.setFont("Helvetica-Bold", 8)
        c.drawCentredString(sx + sig_w/2, sig_y + sig_h - 7, lbl)
        c.setFillColor(colors.HexColor("#3a4468"))
        c.setFont("Helvetica", 6)
        c.drawString(sx + 4, sig_y + 4, "Signature:")
        c.drawString(sx + sig_w*0.55, sig_y + 4, "Date/Time:")
        # signature line
        c.setStrokeColor(_PDF_MUTED)
        c.setLineWidth(0.4)
        c.line(sx + 4 + 28, sig_y + 4, sx + sig_w*0.52 - 2, sig_y + 4)
        c.line(sx + sig_w*0.55 + 28, sig_y + 4, sx + sig_w - 4, sig_y + 4)

    # ── page footer ───────────────────────────────────────────────────────────
    c.setFillColor(_PDF_MUTED)
    c.setFont("Helvetica", 7)
    c.drawCentredString(PAGE_W / 2, 6 * mm,
                        "Page 2 of 2  ·  AIRLoad A350-1000 Cargo Configuration  ·  CONFIDENTIAL")
    c.setStrokeColor(_PDF_BORDER)
    c.setLineWidth(0.3)
    c.line(MARGIN, 10 * mm, PAGE_W - MARGIN, 10 * mm)
...
class AIRLoadDashboard(tk.Tk):
    ...
    def _update_live_cg(self):
        ...
        # Don't set a WARNING status until the user has placed at least one cargo item
        if not self._cg_active():
            self.plan["status"] = "AWAITING CARGO"
        else:
    ...
    def _cg_active(self) -> bool:
        """
        Returns True only when CG values should be displayed — i.e. when at
        least one cargo item has been assigned to a zone, OR an Auto-Recommend
        result is currently active.  Until then all CG readouts show '—' so
        the user does not see a spurious out-of-limits warning on an empty
        (no cargo) flight plan.
        """
        if self._active_recommendation is not None:
            return True
        return any(e.get("assigned_zone") for e in self.plan.get("loading_plan", []))
    ...
    def _build_status_bar(self):
        for w in self.status_bar.winfo_children():
            w.destroy()
        plan     = self.plan
        cg_active = self._cg_active()

        # When no cargo is loaded yet, show a neutral "AWAITING CARGO" pill
        # instead of any CG-derived status (which would falsely warn).
        if cg_active:
        ...
        cg_col   = C[compute_cg_status(final_cg)] if cg_active else C["muted"]
        ...
        _kv("Final CG",
            f"{final_cg:.3f} % MAC" if (cg_active and final_cg is not None) else "—",
            cg_col)
        ...
        # Only show infeasibility warnings after cargo has been loaded
        if cg_active:
    ...
    def _build_cg_info_bar(self):
        ...
        if cg_active and final_cg is not None:
            dev = final_cg - CG_TGT
            _tile("CG Deviation", f"{dev:+.3f} % vs TGT", C[compute_cg_status(final_cg)])
        else:
            _tile("CG Deviation", "—", C["muted"])
        ...
        if steps:
            if cg_active:
                ...
                _tile("CG @ TOW",     "—", C["muted"])
                _tile("CG @ LDW",     "—", C["muted"])
                _tile("Min CG flight","—", C["muted"])
                _tile("Max CG flight","—", C["muted"])
    ...
    def _build_sidebar(self, parent):
        ...
        # Shown only when all cargo items have been assigned a zone.
        all_assigned = all(e.get("assigned_zone") for e in self.plan.get("loading_plan", []))
        any_items    = len(self.plan.get("loading_plan", [])) > 0
        if all_assigned and any_items:
            tk.Frame(parent, bg=C["border"], height=1).pack(fill="x", padx=8, pady=(10, 0))
            dl_frame = tk.Frame(parent, bg=C["panel"], pady=8, padx=8)
            dl_frame.pack(fill="x")
            tk.Label(dl_frame,
                     text="✓ All cargo assigned",
                     font=(UI_FONT, 9, "bold"),
                     fg=C["green"], bg=C["panel"]).pack()
            tk.Label(dl_frame,
                     text="Export configuration for load master sign-off:",
                     font=(UI_FONT, 8),
                     fg=C["muted"], bg=C["panel"],
                     justify="center").pack(pady=(2, 6))
            tk.Button(dl_frame,
                      text="⬇  Download JSON  (Log File)",
                      command=self._download_json,
                      bg=C["panel2"], fg=C["accent"],
                      font=(UI_FONT, 9, "bold"),
                      relief="flat", padx=10, pady=5,
                      cursor="hand2", width=26).pack(pady=(0, 4))
            tk.Button(dl_frame,
                      text="🖨  Download PDF  (Load Sheet)",
                      command=self._download_pdf,
                      bg=C["green"], fg=C["bg"],
                      font=(UI_FONT, 9, "bold"),
                      relief="flat", padx=10, pady=5,
                      cursor="hand2", width=26).pack()
    ...
    def _download_json(self):
        """Export the current cargo configuration as a JSON log file."""
        flt = self.plan.get("flight_plan") or {}
        fn  = (f"AIRLoad_{flt.get('flight_number','CARGO').replace(' ','')}"
               f"_{flt.get('origin','')}{flt.get('destination','')}"
               f"_{datetime.datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json")
        ...
        if not path:
            return
        try:
            payload = build_export_json(self.plan)
            with open(path, "w", encoding="utf-8") as f:
                json.dump(payload, f, indent=2, ensure_ascii=False)
            self._flash_msg(f"✓ JSON saved: {os.path.basename(path)}", C["green"])
        except Exception as exc:
            messagebox.showerror("Export Error", f"Could not save JSON:\n{exc}")

    def _download_pdf(self):
        """Export the current cargo configuration as a landscape PDF load sheet."""
        if not _REPORTLAB_OK:
            ...
            return
        flt = self.plan.get("flight_plan") or {}
        fn  = (f"AIRLoad_LoadSheet_{flt.get('flight_number','CARGO').replace(' ','')}"
               f"_{flt.get('origin','')}{flt.get('destination','')}"
               f"_{datetime.datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.pdf")
        path = filedialog.asksaveasfilename(
            title="Save PDF Load Sheet",
            initialfile=fn,
        ...
        if not path:
            return
        try:
            self._flash_msg("Generating PDF…", C["accent"])
            self.update_idletasks()
            generate_pdf_loadsheet(self.plan, self._item_colours, path)
            self._flash_msg(f"✓ PDF saved: {os.path.basename(path)}", C["green"])
        except Exception as exc:
    ...
    def _draw_aircraft(self):
        ...
        if final_cg is not None and self._cg_active():
